# Gym Helper — Vite + React

Мобильное веб-приложение для тренировок, профиля, веса, питания и облачной синхронизации.

## Запуск локально

```bash
npm install
npm run dev
```

## Деплой на Vercel

1. Загрузи файлы проекта в GitHub.
2. Подключи репозиторий к Vercel.
3. Framework Preset: `Vite`.
4. Build Command: `npm run build`.
5. Output Directory: `dist`.

## Авторизация и облачная синхронизация через Supabase

Авторизация уже встроена в приложение. Без Supabase приложение продолжает работать локально через `localStorage`.

### 1. Создай проект Supabase

Создай проект на Supabase и возьми:

- Project URL / API URL: `https://...supabase.co`
- Publishable key / anon public key

### 2. Создай таблицу и включи Realtime

Открой Supabase Dashboard -> SQL Editor -> New query и выполни SQL из файла:

```text
supabase-schema.sql
```

Если таблица уже была создана ранее, можно выполнить только:

```text
supabase-realtime.sql
```

Таблица называется `app_state`. В ней хранится один JSON-документ на пользователя: профиль, тренировки, вес, питание, избранные продукты и сохраненные меню.

### 3. Добавь переменные окружения в Vercel

В Vercel открой проект -> Settings -> Environment Variables и добавь:

```text
VITE_SUPABASE_URL=твой Project URL
VITE_SUPABASE_ANON_KEY=твой Publishable key / anon public key
```

После этого сделай Redeploy.

### 4. Как работает синхронизация

- До входа данные хранятся локально в браузере.
- После входа локальные данные объединяются с облаком.
- После этого изменения автоматически сохраняются в Supabase.
- На другом устройстве данные обновляются через Supabase Realtime без ручного обновления страницы.
- При одновременном редактировании на двух устройствах побеждает последняя сохраненная версия.

## Брендированное письмо подтверждения

В файле `supabase-email-template.md` есть готовый шаблон письма Gym Helper.

Обычно путь такой:

```text
Supabase -> Authentication -> Email Templates -> Confirm signup
```

Туда можно вставить subject и HTML из файла. Для красивого имени отправителя `Gym Helper` лучше настроить Custom SMTP в Supabase.

## Штрихкод и питание

Сканер ищет продукт в Open Food Facts и заполняет КБЖУ на 100 г. После сканирования выбери граммовку и прием пищи, затем нажми `Добавить в дневник`. Если в Open Food Facts нет полного КБЖУ, введи данные с этикетки вручную.

## Основные файлы

- `src/main.jsx` — логика приложения.
- `src/styles.css` — внешний вид.
- `public/exercises` — картинки упражнений.
- `supabase-schema.sql` — таблица, RLS-политики и Realtime.
- `supabase-realtime.sql` — только включение Realtime для уже созданной таблицы.
- `supabase-email-template.md` — шаблон письма подтверждения.

## Что добавлено в v10

- Адаптивная версия для компьютера и планшета: на широком экране появляется боковая навигация, а контент получает больше места.
- В профиле можно задавать свои цели по калориям, белкам, жирам и углеводам. Пустые поля означают автоматический расчет.
- В питании добавлены пресеты меню и конструктор рациона: можно дать меню название, собрать его из продуктов, сохранить, применить или удалить.
- График веса дополнительно разнесен по подписям, даты выводятся числовым форматом `20/05/26`.

SQL в Supabase менять не нужно: новые поля хранятся в том же JSON-состоянии `app_state`.


## v10 fixes
- Исправлена потеря локальных данных при перезагрузке/выходе: localStorage больше не перезаписывается пустым состоянием до загрузки.
- Свои продукты с этикетки автоматически сохраняются в общий список продуктов при добавлении в дневник.
- Добавлено удаление вручную сохраненных продуктов из общего списка.


## v12 stable navigation

Навигация восстановлена: четыре основных раздела — Сегодня, Трен, Питание, Профиль. Верхний дублирующий переключатель удален.

## v14 — этап 1 стабилизации

- Добавлен ErrorBoundary: если отдельный экран упадет, приложение покажет понятный экран ошибки вместо белого экрана.
- Профиль теперь редактируется через временную копию: изменения применяются только после кнопки «Сохранить», а «Отмена» откатывает правки.
- Добавлена кнопка «Выйти и очистить устройство» для удаления локальных данных на общем устройстве.
- BMR/TDEE больше не считаются, пока пользователь не выбрал пол, чтобы не подставлять неверную формулу по умолчанию.

Supabase-схему менять не нужно.

## v19 — этап 5: аналитика недели и PWA

- На главном экране появился блок «Неделя» с калориями по дням, тренировочными днями, объемом силовых, кардио, средним белком и динамикой веса.
- Добавлена базовая PWA-поддержка: manifest с иконками, service worker и карточка установки приложения на экран.
- На Android/Chrome может появиться кнопка установки. На iPhone установка делается через Safari -> Поделиться -> «На экран Домой».

Supabase-схему менять не нужно.
